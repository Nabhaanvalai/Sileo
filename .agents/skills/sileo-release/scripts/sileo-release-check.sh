...
if ! .github/scripts/changelog-section.sh "$version" >/tmp/sileo-release-notes.md; then
  echo "Could not extract CHANGELOG.md section for $version" >&2
  exit 1
fi

if [[ ! -s /tmp/sileo-release-notes.md ]]; then
  echo "Extracted changelog section is empty for $version" >&2
  exit 1
fi

if ! grep -q "^## \\[$version\\]" /tmp/sileo-release-notes.md; then
...
echo
cat /tmp/sileo-release-notes.md
